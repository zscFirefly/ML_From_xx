...
def show_scatter(a):
    ...
    ax.set_ylabel("绿色分量")
    ax.set_zlabel("蓝色分量")
    plt.title("图像颜色三维频数分布",fontsize=15)
    ...
    plt.title("图像颜色频数分布",fontsize=15)
    plt.grid(True)
...
if __name__ == "__main__":
    ...
    # 颜色数量
    num_vq = 2
    im = Image.open("16.Lena.png") # 16.Lena.png #16.flower2.png #16.son.png
    image = np.array(im).astype(np.float)/255
    image = image[:,:,:]
    image_v = image.reshape((-1,3)) #将三维矩阵变成二维矩阵
    model = KMeans(num_vq)
    ...
    plt.axis("off")
    plt.title("矢量化后图片：%d色"%num_vq,fontsize=18)
    plt.imshow(vq_image)
