...
if __name__ == "__main__":
    N = 100
    x = np.random.rand(N) * 6 - 3 #生成均值为0的均匀分布
    x.sort()
    y = np.sin(x) + np.random.randn(N) * 0.05 #均值是0的100个样本
    x = x.reshape(-1,1) #转置得二维数组


    reg = DecisionTreeRegressor(criterion="mse",max_depth=9,) # mse 均方误差
    dt = reg.fit(x,y)
    ...
    plt.legend(loc='upper left')
    plt.title("决策树在不同深度下的回归情况",fontsize = 15)
    plt.subplots_adjust(top=0.92)
    plt.grid()
    plt.show()