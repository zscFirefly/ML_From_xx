...
if __name__ == "__main__":
    ...
    plt.colorbar(cmesh,shrink=0.8) #shrink调高度
    ...
    for i,cc in enumerate(zip(centers,covs)):
        ...
        e = Ellipse(xy=center,width = width,height = height,angle=angle,color='m',alpha=0.5,clip_box=ax.bbox)
        ax.add_artist(e)
    plt.xlim((x1_min,x1_max))
    plt.ylim((x2_min,x2_max))
    mpl.rcParams['font.sans-serif'] = ['SimHei']
    mpl.rcParams['axes.unicode_minus'] = False
    plt.title("Gmm似然函数值",fontsize=20)
    plt.grid(True)
    plt.show()
