...
if __name__ == "__main__":
    ...
    types = ("spherical","diag","tied","full") # sperical圆形;diag:对角线;tied:两个数据
    ...
    plt.title("不同方差类型的误差率和BIC",fontsize=18)
    plt.show()

    # bic越小越好，故选取最小的bic做我参数，再次预测，并画图
    optimal = bic.argmin()
    ...
    plt.scatter(x[:,0],x[:,1],s=30,c=y,marker='o',cmap=cm_dark,edgecolor = 'k')

    ax1_min,ax1_max,ax2_min,ax2_max = plt.axis()
    plt.xlim((x1_min,x1_max))
    plt.ylim((x2_min,x2_max))
    plt.title("GMM调参：covariance_type=%s"%types[optimal],fontsize=20)
    plt.grid()
    plt.show()
